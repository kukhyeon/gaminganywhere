...
filter_RGB2YUV_threadproc(void *arg) {
	...
	while(filter_started != 0) {
		...
		if(iid == 0) {
			char individual_frames[128];
			// Check if save-individual-frames is configured
			if(ga_conf_readv("save-individual-frames", individual_frames, sizeof(individual_frames)) != NULL) {
				static int server_frame_count = 0;
				char server_frame_filename[256];
				FILE *server_frame_fp;
				
				snprintf(server_frame_filename, sizeof(server_frame_filename), "server_frame_%08d.yuv", server_frame_count);
				
				// Save individual YUV file
				server_frame_fp = fopen(server_frame_filename, "wb");
				if(server_frame_fp != NULL) {
					ga_save_yuv420p(server_frame_fp, outputW, outputH, dst, dstframe->linesize);
					fclose(server_frame_fp);
					ga_error("Saved server frame: %s\n", server_frame_filename);
				}
				
				server_frame_count++;
			}
			
			// Also save to continuous file if savefp is set
			if(savefp != NULL) {
