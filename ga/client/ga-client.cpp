...
create_overlay(struct RTSPThreadParam *rtspParam, int ch) {
	...
	// 윈도우 위치 설정 (설정 파일에서 읽어오기)
	char buf[64];
	int window_x = SDL_WINDOWPOS_CENTERED;
	int window_y = SDL_WINDOWPOS_CENTERED;

	if(ga_conf_readv("window-x", buf, sizeof(buf)) != NULL) {
		window_x = ga_conf_readint("window-x");
	}
	if(ga_conf_readv("window-y", buf, sizeof(buf)) != NULL) {
		window_y = ga_conf_readint("window-y");
	}
