...
// --- RTT Client (Port 55556) ---, 20251229
#pragma pack(push, 1)
typedef struct {
	uint32_t seq;
	uint32_t sec;
...
rtt_echo_threadproc(void *arg) {
	...
	if (bind(s, (struct sockaddr*)&si_me, sizeof(si_me)) == -1) {
		rtsperror("RTT client: bind failed (port 55556 busy?)\n");
	}
...
void init_feedback_client(const char *url) {
	...
	// Parse URL: rtsp://192.168.1.100:8554/...
	const char *p = url;
...
//
	...
	// Initialize feedback client, 20251216
	init_feedback_client(rtspParam->url);
	init_rtt_client(rtspParam->url); //20251229
	//
...
DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
	...
	if(fSubsession.rtpPayloadFormat() == video_sess_fmt && frameSize >= 4) {
		...
		// Send feedback immediately, 20251216
